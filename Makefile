...
e2e-test:
	...
	docker run --network test-network -d -e NODE_ENV=development -e SERVICEACCT_TOKEN=eyMt4tP69ZGIh9f120Tw4VdHJIM82twVq-nx4BiFgbw -e headerUrl=$(headerUrl) -e OAUTH2_REDIRECT_URL=$(OAUTH2_REDIRECT_URL) -e grcUiApiUrl=10.10.0.5:4000/grcuiapi -e OAUTH2_CLIENT_ID=$(OAUTH2_CLIENT_ID) -e OAUTH2_CLIENT_SECRET=$(OAUTH2_CLIENT_SECRET) -e API_SERVER_URL=$(API_SERVER_URL) --name grcui  -t -i quay.io/open-cluster-management/grc-ui:3.6.0-PR36-fcc3b830b0c0cf89cb26f5d04da254aca2261c10
