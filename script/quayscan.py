...
log = logging.getLogger('quayscan')
...
def getScanReport(host, port, token, namespace, imageName, imageTag):
    ...
    if not scan_result:
        return {'status': 'failed', 'result': 'no quayscan report'}
...
def main():
    global MANIFEST_DIGEST
    MANIFEST_DIGEST = None
    parser = argparse.ArgumentParser(description="Check image quan scan report")
    parser.add_argument("--quayscan_access_token", default=None, help="Quay API acess token")
    parser.add_argument("--image", help="The image you want to check the result of the vulnerability scan")

    args = parser.parse_args()
    token = args.quayscan_access_token
    image = args.image
    port = 443
