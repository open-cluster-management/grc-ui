...
class GrcApolloClient {
  ...
  // getTopologyFilters(){
  //   return grcClient.query({ query: Query.HCMTopologyFilters })
  // }

  createResource(resourceType, input){
    switch (resourceType.name) {
    case RESOURCE_TYPES.HCM_REPOSITORIES.name:
      return grcClient.mutate({ mutation: Query.HCMSetRepository, variables: { repo: input } })
    default:
  ...
  installHelmChart(input) {
    return grcClient.mutate({ mutation: Query.HCMInstallHelmChart, variables: { input } })
  }
