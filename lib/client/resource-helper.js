...
export const transform = (resource, key, locale, isSearch) => {
  ...
  if (key.type === 'timestamp') {
    ...
    return data ? data.map(tagText => <Tag key={_uniqueId('tag')} style={{display: 'inline-block'}} type={'beta'} title={tagText.title}>{tagText.value ? `${tagText.name}:${tagText.value}` : tagText.name}</Tag>) : '-'
