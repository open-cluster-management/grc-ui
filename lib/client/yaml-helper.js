...
class YamlInline {
  ...
  parseQuotedScalar(scalar, i) {
    ...
    //let item = /^(.*?)['"]\s*(?:[,:]|[}\]]\s*,)/.exec((scalar+'').substring(i))[1];
...
class YamlParser {
  ...
  parse(value, row) {
    ...
    while (this.moveToNextLine()) {
      ...
      isSequence = !!values
      if (!isSequence) {
        values = new RegExp('^(' + YamlInline.REGEX_QUOTED_STRING + '|[^ \'"[{].*?) *:(\\s+(.+?))?\\s*$').exec(this.currentLine)
        isMapping = !!values
      }
      if (isSequence) {
        ...
        const flag1 = (!this.isDefined(values.value) || '' === this.trim(values.value) || values.value.replace(/^ +/, '').charAt(0) === '#')
        const flag2 = (this.isDefined(values.leadspaces) && ' ' === values.leadspaces && (matches))
        if (flag1) {
          c = this.getRealCurrentLineNb() + 1
          parser = new YamlParser(c, this.lined)
          parser.refs = this.refs
          data.push(this.getRealCurrentLineNb({
            $r: c,
        ...
        // hash
          ...
          const obj = this.getRealCurrentLineNb({})
  ...
  isCurrentLineComment() {
    const ltrimmedLine = this.currentLine.replace(/^ +/g, '')
  ...
  cleanup(value) {
    ...
    regex = /^(#.*?\n)+/
    if (regex.test(value)) {
      const trimmedValue = value.replace(regex, '')
