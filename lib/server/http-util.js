...
exports.getOptions = function(req, getOptionsUrL) {
  return {
    url: getOptionsUrL,
    qs: req.query
  }
...
function redactBody(body) {
  if (typeof body === 'string') {
    // don't use *+{ more than once each time to avoid ReDoS warning
    // still keep the same original logic and remove eslint-disable
    const replace = '$1"***"'
    return body.replace(/"[^"]*password"/gi, replace).
      replace(/"[^"]*access_token"/gi, replace).
      replace(/\$1"\*{3}":"[^"]*"/gi, replace)
      //here above first * is escaped, just matching replace = '$1"***"'
  }
  return body
}
  ...
  try {
    ...
    if (query) {
      ...
      for (const i in queryObj) {
        if (Object.prototype.hasOwnProperty.call(queryObj, i) && i === 'token') {
          queryObj[i] = '***'
        }
      }
