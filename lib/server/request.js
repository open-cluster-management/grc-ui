...
import { serializeRequest, requestUrl, serializeResponse } from './http-util'
import request, { RetryStrategies, defaults } from 'requestretry'
 ...
 */
export default function (options, req, acceptedStatusCodes, callback, logger, slowThreshold, reqOptions) {
  let calledFrom
  request = defaults(reqOptions || REQUEST_DEFAULTS)
  ...
  if (logger) {
    calledFrom = new Error('Called From:')
  ...
  request(options, (error, res, body) => {
    ...
    if (acceptedStatusCodes.indexOf(res.statusCode) !== -1) {
      try {
        ...
        if (logger) {
          logger.error([
            'Error processing response from ', (options.method || 'GET'), ' ', requestUrl(options), '\n',
            error3.stack, '\n',
            calledFrom ? calledFrom.stack : ''
          ].join(''))
        }
        if (req && req.res) {
          const res2 = req.res
...
function logRequest(options, logger, requestId) {
  if (logger.isDebugEnabled()) {
    const requestStr = serializeRequest(options)
...
function logResponse(res, options, logger, requestId, duration) {
  if (logger.isDebugEnabled()) {
    const responseStr = serializeResponse(res)
    logger.debug('response ' + requestId + ' - ' + duration + ' ms\n' + responseStr)
  } else if (logger.isInfoEnabled()) {
    logger.info([(options.method || 'GET'), ' ', requestUrl(options), ' ', res.statusCode, ' HTTP/1.1 (', duration, ' ms)'].join(''))
  }
}

function logSlowResponse(options, res, logger, duration) {
  logger.warn('Slow response from request ' + requestUrl(options) + ' (' + duration + ' ms)')
}
