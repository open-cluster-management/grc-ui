...
export headerUrl=https://`oc get route multicloud-console -n $installed_namespace -o=jsonpath='{.spec.host}'`
...
make docker/login
export DOCKER_URI=quay.io/stolostron/grc-ui-api:${GRCUIAPI_VERSION:-"latest-dev"}
make docker/pull
