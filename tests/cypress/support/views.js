...
export const action_scheduleAutomation = (uName, credentialName, mode) => {
  ...
  cy.get('.createCredential').within(() => {
    ...
    cy.contains(credentialName).should('be.visible')
    cy.waitUntil(() => cy.get(credentialName).should('be.visible'))
    cy.get(credentialName).should('be.visible').click()
  ...
  if (mode === 'manual') {
    cy.get('.ansible-configure-table').within(() => {
      cy.waitUntil(() => cy.get('.pf-c-radio__label').eq(0).should('be.visible'))
      cy.get('.pf-c-radio__label').eq(0).should('be.visible').click()
      failures = 2
    })
  } else if (mode === 'once') {
    cy.get('.ansible-configure-table').within(() => {
      cy.waitUntil(() => cy.get('.pf-c-radio__label').eq(1).should('be.visible'))
      cy.get('.pf-c-radio__label').eq(1).should('be.visible').click()
      failures = 1
    })
  } else {
    cy.get('.ansible-configure-table').within(() => {
      cy.waitUntil(() => cy.get('.pf-c-radio__label').eq(2).should('be.visible'))
      cy.get('.pf-c-radio__label').eq(2).should('be.visible').click()
      failures = 0
    })
  }
